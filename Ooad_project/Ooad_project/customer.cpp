...
Shop get_shop(ulli Shop_id) {
    Shop temp;
    string id = Utilities::ulli_to_string(Shop_id);
...
public:
    ...
    int get_details(string email) {
        ...
        cout << "> Enter your phone number (with country code, e.g., +91xxxxxxxxxx) :: ";
        string phone_number;
        getline(cin, phone_number);

        if (phone_number[0] != '+' || phone_number.length() < 12) {
            cout << "> Invalid format. Phone number must start with + and include country code.\n";
            return -1;
        }

        for (size_t i = 1; i < phone_number.length(); ++i) {
            if (!isdigit(phone_number[i])) {
                cout << "> Invalid phone number: must contain only digits after +.\n";
                return -1;
            }
        }
        ...
        cout << "> It must contain at least 1 number, capital letter, small letter and alphabet. Length of password must also be >= 6\n";
        cout << "> Password :: ";
    ...
    void add_cart(Transaction &t, ulli shop_ID) {
        ...
        display_shop_details(shop.category);
        ...
        do {
            ...
            if (number_of_items > 0) {
                ...
                t.total_price += (item.price * number_of_items);
                item.quantity -= number_of_items;
